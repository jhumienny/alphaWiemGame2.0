...
function App() {
  ...
  function toggleCategory(id) {
    setSelectedCategoryIds((current) => current.includes(id) ? current.filter((selectedId) => selectedId !== id) : [...current, id])
  }
  ...
  const filteredLibrary = useMemo(() => library.filter((item) => item.text.toLowerCase().includes(query.toLowerCase()) && (!selectedCategoryIds.length || selectedCategoryIds.some((id) => id === '__unassigned__' ? !item.categoryId : item.categoryId === id))), [library, query, selectedCategoryIds])
...
function CategoryFilter({ containerRef, categories, selectedIds, isOpen, newCategoryName, onToggle, onSelect, onClear, onNameChange, onAdd }) {
  const selectedLabel = selectedIds.length === 1 ? (selectedIds[0] === '__unassigned__' ? 'Bez kategorii' : categories.find((category) => category.id === selectedIds[0])?.name) : selectedIds.length > 1 ? `Kategorie (${selectedIds.length})` : 'Wszystkie kategorie'
  const option = (id, label) => <label className={`category-filter-option ${selectedIds.includes(id) ? 'is-selected' : ''}`} key={id}><input type="checkbox" checked={selectedIds.includes(id)} onChange={() => onSelect(id)} /><span>{label}</span></label>
  return <div className="category-filter" ref={containerRef}><button className="category-filter-toggle" type="button" aria-label="Filtruj według kategorii" aria-expanded={isOpen} onClick={onToggle}><span>{selectedLabel}</span>{selectedIds.length ? <span className="category-filter-clear" role="button" aria-label="Wyczyść filtr kategorii" tabIndex="0" onClick={(event) => { event.stopPropagation(); onClear() }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onClear() } }}>×</span> : <span className="category-filter-arrows" aria-hidden="true"><svg viewBox="0 0 12 12"><path d="m3 4 3-3 3 3" /><path d="m3 8 3 3 3-3" /></svg></span>}</button>{isOpen && <div className="category-filter-panel"><p>Wszystkie kategorie</p>{option('__unassigned__', 'Bez przypisanej kategorii')}{categories.map((category) => option(category.id, category.name))}<form onSubmit={onAdd}><input aria-label="Nazwa nowej kategorii" autoComplete="off" name="new-question-category" data-1p-ignore="true" value={newCategoryName} onChange={(event) => onNameChange(event.target.value)} placeholder="Nowa kategoria" /><button type="submit">Dodaj kategorię</button></form></div>}</div>
}
